...
def ensure_pathways_schema() -> None:
    with _connect_db() as conn:
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS pathways (
                ...
                name TEXT NOT NULL,
                admission_md TEXT NOT NULL DEFAULT '',
        ...
        # Migration: rename content_md → admission_md + add progress_md
        try:
            conn.execute("ALTER TABLE pathways ADD COLUMN admission_md TEXT NOT NULL DEFAULT '';")
        except sqlite3.OperationalError:
            pass
        try:
            conn.execute("ALTER TABLE pathways ADD COLUMN progress_md TEXT NOT NULL DEFAULT '';")
        except sqlite3.OperationalError:
            pass
        # Copy old content_md into admission_md if the old column exists
        try:
            conn.execute("UPDATE pathways SET admission_md = content_md WHERE admission_md = '' AND content_md != '';")
        except sqlite3.OperationalError:
            pass
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS pathway_citations (
                pathway_id TEXT NOT NULL,
                cite_key INTEGER NOT NULL,
                item_type TEXT NOT NULL CHECK(item_type IN ('abstract','guideline')),
                item_id TEXT NOT NULL,
                PRIMARY KEY (pathway_id, cite_key)
            );
            """
        )
...
def list_pathways() -> List[Dict[str, str]]:
    with _connect_db() as conn:
        rows = conn.execute(
            "SELECT pathway_id, name FROM pathways ORDER BY name;"
        ).fetchall()
    return [{"pathway_id": r["pathway_id"], "name": r["name"]} for r in rows]


def get_pathway(pathway_id: str) -> Dict[str, str]:
    pid = (pathway_id or "").strip()
    if not pid:
        return {}
    with _connect_db() as conn:
        row = conn.execute(
            "SELECT pathway_id, name, admission_md, progress_md, created_at, updated_at FROM pathways WHERE pathway_id=? LIMIT 1;",
            (pid,),
        ).fetchone()
    if not row:
        return {}
    return {
        "pathway_id": (row["pathway_id"] or "").strip(),
        "name": (row["name"] or "").strip(),
        "admission_md": row["admission_md"] or "",
        "progress_md": row["progress_md"] or "",
        "created_at": (row["created_at"] or "").strip(),
        "updated_at": (row["updated_at"] or "").strip(),
    }


def get_pathway_by_name(name: str) -> Dict[str, str]:
    n = (name or "").strip()
    if not n:
        return {}
    with _connect_db() as conn:
        row = conn.execute(
            "SELECT pathway_id, name, admission_md, progress_md, created_at, updated_at FROM pathways WHERE name=? LIMIT 1;",
            (n,),
        ).fetchone()
    if not row:
        return {}
...
def create_pathway(name: str) -> str:
    n = (name or "").strip()
    if not n:
    ...
    with _connect_db() as conn:
        conn.execute(
            "INSERT INTO pathways (pathway_id, name, admission_md, progress_md, created_at, updated_at) VALUES (?,?,?,?,?,?);",
            (pid, n, "", "", now, now),
        )
    return pid


def update_pathway_content(pathway_id: str, admission_md: str, progress_md: str) -> None:
    pid = (pathway_id or "").strip()
    if not pid:
        return
    now = _utc_iso_z()
    with _connect_db() as conn:
        conn.execute(
            "UPDATE pathways SET admission_md=?, progress_md=?, updated_at=? WHERE pathway_id=?;",
            (admission_md, progress_md, now, pid),
        )


def delete_pathway(pathway_id: str) -> None:
    pid = (pathway_id or "").strip()
    if not pid:
        return
    with _connect_db() as conn:
        conn.execute("DELETE FROM pathway_citations WHERE pathway_id=?;", (pid,))
        conn.execute("DELETE FROM pathways WHERE pathway_id=?;", (pid,))


def list_pathway_citations(pathway_id: str) -> List[Dict[str, str]]:
    pid = (pathway_id or "").strip()
    if not pid:
        return []
    with _connect_db() as conn:
        rows = conn.execute(
            "SELECT cite_key, item_type, item_id FROM pathway_citations WHERE pathway_id=? ORDER BY cite_key;",
            (pid,),
        ).fetchall()
    return [
        {"cite_key": str(r["cite_key"]), "item_type": r["item_type"], "item_id": r["item_id"]}
        for r in rows
    ]


def add_pathway_citation(pathway_id: str, item_type: str, item_id: str) -> int:
    """Add a citation and return the assigned cite_key."""
    pid = (pathway_id or "").strip()
    if not pid:
        return 0
    with _connect_db() as conn:
        row = conn.execute(
            "SELECT COALESCE(MAX(cite_key), 0) AS mx FROM pathway_citations WHERE pathway_id=?;",
            (pid,),
        ).fetchone()
        next_key = (row["mx"] if row else 0) + 1
        conn.execute(
            "INSERT INTO pathway_citations (pathway_id, cite_key, item_type, item_id) VALUES (?,?,?,?);",
            (pid, next_key, item_type, item_id),
        )
    return next_key


def remove_pathway_citation(pathway_id: str, cite_key: int) -> None:
    """Remove a citation and re-number remaining ones."""
    pid = (pathway_id or "").strip()
    if not pid:
        return
    with _connect_db() as conn:
        conn.execute(
            "DELETE FROM pathway_citations WHERE pathway_id=? AND cite_key=?;",
            (pid, cite_key),
        )
